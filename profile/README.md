# Cowork
<img width="1512" height="947" alt="image" src="https://github.com/user-attachments/assets/db8ccf6b-98ef-4a9d-a8ac-ebaf43f5404c" />
...
Cowork는 이러한 학생회 행정을 **행사**와 연결된 예산, 파일, 회의록, 설문, 일정, 지출 증빙을 하나의 흐름 안에서 관리할 수 있도록 설계되었습니다.
...
학생회에서 진행하는 행사를 등록하고, 행사와 연결된 파일, 예산, 사진, 설문, 일정 조율, 회의록을 한 곳에서 관리할 수 있습니다.  
행사별로 진행 상태와 자료 누락 여부를 확인할 수 있어, 행사 준비부터 종료 후 정산까지의 흐름을 체계적으로 관리할 수 있습니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/432ac3d0-3791-4f89-aa9e-d55fd1f3e954" />


- 행사 목록 화면에서는 전체 행사, 기획중, 진행중, 완료, 취소, 누락 상태별로 행사를 필터링할 수 있습니다.  
각 행사 카드에는 행사명, 날짜, 장소, 담당자, 계획 예산이 함께 표시되며, 자료가 누락된 행사는 별도 상태로 표시하여 후속 작업이 필요한 행사를 빠르게 확인할 수 있습니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/89fd2b32-8b25-4944-8d9c-84307ae61851" />


- 행사 상세 화면에서는 행사 기본 정보와 함께 관련 자료 현황을 요약해서 확인할 수 있습니다.  
총 지출, 관련 파일 수, 사진 수, 설문 응답 수, 회의록 수를 카드 형태로 보여주어 행사 운영에 필요한 자료가 얼마나 등록되어 있는지 한눈에 파악할 수 있습니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/52f53b94-b6a1-4695-b6d1-bc65a3ad784d" />


- 예산 탭에서는 행사와 연결된 지출 내역을 확인할 수 있습니다.  
날짜, 카테고리, 거래처, 지출 내용, 영수증 첨부 여부, 증빙 사진 수, 금액을 함께 보여주며, 하단에는 행사별 총 지출 금액을 집계하여 표시합니다.

구현 포인트

- Event 도메인을 중심으로 파일, 예산, 사진, 설문, 일정 조율, 회의록 데이터를 연결했습니다.
- 행사 상태를 기획중, 진행중, 완료, 취소, 누락 등으로 구분하여 운영 상황을 명확하게 관리할 수 있도록 구성했습니다.
- 행사 목록에서는 상태 필터와 카드형 UI를 통해 사용자가 필요한 행사를 빠르게 찾을 수 있도록 했습니다.
- 행사 상세 화면에서는 각 탭별 데이터 개수를 집계하여 행사 관련 자료의 등록 현황을 한눈에 확인할 수 있도록 했습니다.
- 예산 탭에서는 행사에 연결된 지출 내역을 조회하고, 영수증 및 증빙 사진 첨부 여부를 함께 확인할 수 있도록 구성했습니다.
- 행사별 총 지출 금액을 집계하여 계획 예산과 실제 지출을 비교할 수 있는 기반을 마련했습니다.
- 자료 누락 상태를 별도로 표시하여 행사 종료 후 정산, 회의록, 사진, 파일 등록 등의 후속 작업을 놓치지 않도록 했습니다.

### 4.2 파일 관리

학생회 운영 과정에서 발생하는 기획서, 결과보고서, 예약 확인서 등의 파일을 행사별로 연결하여 관리할 수 있습니다.  
파일은 전체 목록으로 확인할 수도 있고, 특정 행사 기준으로 필터링하여 해당 행사에 필요한 자료만 따로 조회할 수 있습니다.

<img width="1536" height="998" alt="image" src="https://github.com/user-attachments/assets/b33883ef-b1b1-443c-ad41-1e94b7e5422c" />

- 전체 파일 화면에서는 등록된 파일을 한 번에 확인할 수 있습니다.  
각 파일은 파일명, 연결된 행사, 파일 크기, 수정일, 업로드한 사람 정보를 함께 보여주며, 검색 기능을 통해 필요한 문서를 빠르게 찾을 수 있습니다.

<img width="1536" height="998" alt="image" src="https://github.com/user-attachments/assets/86c0415b-4c16-4fed-b783-99fc53a76f7d" />

- 행사별 분류 기능을 통해 특정 행사에 연결된 파일만 따로 조회할 수 있습니다.  
예를 들어 `2026 상반기 MT` 행사를 선택하면 해당 행사와 연결된 기획안, 예약 확인서 등의 파일만 확인할 수 있으며, 상단의 행사 페이지 링크를 통해 행사 상세 화면으로 이동할 수 있습니다.

<img width="1536" height="998" alt="image" src="https://github.com/user-attachments/assets/cd7043cd-ac76-41c1-b1d1-4f9cdeca7fe8" />

- 파일 업로드 화면에서는 파일을 드래그하거나 클릭하여 업로드할 수 있습니다.  
업로드할 파일은 선택적으로 특정 행사와 연결할 수 있으며, 연결된 행사가 있는 경우 행사 상세 화면에서도 해당 파일을 함께 확인할 수 있습니다.

구현 포인트

- 파일을 단순 저장하는 기능이 아니라, 행사 도메인과 연결하여 관리할 수 있도록 구성했습니다.
- 전체 파일 조회와 행사별 파일 조회를 분리하여 사용자가 필요한 문서를 빠르게 찾을 수 있도록 했습니다.
- 파일명, 파일 크기, 수정일, 업로드 사용자 정보를 함께 저장하여 문서 관리에 필요한 메타데이터를 제공했습니다.
- 업로드 시 특정 행사를 선택적으로 연결할 수 있도록 하여, 행사 상세 화면에서도 관련 파일을 확인할 수 있는 구조로 설계했습니다.
- 검색 기능을 통해 많은 파일 중 필요한 자료를 빠르게 찾을 수 있도록 했습니다.
- 파일 수정 및 삭제 기능을 제공하여 잘못 등록된 문서나 오래된 문서를 관리할 수 있도록 했습니다.
- 미연결 파일 상태를 별도로 관리하여 어떤 행사에도 연결되지 않은 파일을 추적할 수 있도록 했습니다.

### 4.3 예산 처리

학생회 운영 과정에서 발생하는 지출 내역과 영수증을 통합 관리할 수 있습니다.  
사용자는 지출을 직접 등록하거나 통장 거래 내역 엑셀 파일을 업로드하여 기존 지출 내역과 자동으로 매칭할 수 있습니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/7bea38ca-0299-4760-a7f6-7596db209705" />

- 예산 처리 화면에서는 전체 지출 금액과 부서별 지출 현황을 요약해서 확인할 수 있습니다.  
각 지출 내역은 사용일, 부서, 예산 항목, 사용처, 금액, 결제수단, 연결 행사, 영수증 첨부 여부, 증빙 사진 여부를 함께 보여줍니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/5a8233a5-ff49-40b7-9435-bab64dfc8383" />

- 통장 내역 업로드 기능을 통해 `.xlsx`, `.xls` 형식의 거래 내역 파일을 업로드할 수 있습니다.  
업로드된 통장 거래 내역은 이미 등록된 지출 내역과 자동으로 비교되어, 매칭된 거래와 미매칭 거래를 구분합니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/f4db33cc-6417-49b9-9a5c-8f1cd7cd37a5" />

- 통장 매칭 결과 화면에서는 전체 거래 수, 매칭된 거래 수, 미매칭 거래 수를 확인할 수 있습니다.  
거래일시, 가맹점, 금액을 기준으로 등록된 지출 내역과 비교하고, 매칭 여부를 상태값으로 표시하여 회계 검증 작업을 쉽게 진행할 수 있습니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/92df572e-371c-4fd5-a23a-a01ee6ad5383" />

- 매칭 결과는 예산 목록에도 반영됩니다.  
매칭된 지출은 정상 상태로 표시하고, 영수증이나 증빙 사진이 누락된 지출은 별도 상태로 표시하여 후속 보완이 필요한 항목을 빠르게 확인할 수 있습니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/b951c0ae-8d5d-48f6-829c-b405b42e5e46" />

- 지출 등록 화면에서는 집행일, 거래 시간, 부서, 예산 항목, 결제 수단, 사용처, 금액, 사용 내용을 입력할 수 있습니다.  
선택적으로 행사를 연결할 수 있으며, 영수증 파일을 함께 업로드하여 지출 증빙 자료로 관리할 수 있습니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/81282dbf-df96-41b0-a8f5-ee94672d2e74" />

- 모바일 등록 기능을 통해 PC에서 QR 코드를 생성하고, 모바일에서 영수증 사진을 촬영하여 지출 등록을 이어서 진행할 수 있습니다.  
영수증 사진 촬영 후 OCR을 통해 날짜, 금액, 가맹점 정보를 자동으로 추출하고, 추출된 정보를 지출 등록 화면에 반영할 수 있습니다.

구현 포인트

- 지출 내역을 부서, 예산 항목, 결제수단, 연결 행사 기준으로 관리할 수 있도록 구성했습니다.
- 전체 지출 금액과 부서별 지출 금액을 집계하여 예산 사용 현황을 빠르게 확인할 수 있도록 했습니다.
- 각 지출 내역에 영수증과 증빙 사진 첨부 여부를 표시하여 누락된 증빙 자료를 쉽게 추적할 수 있도록 했습니다.
- 지출 내역을 특정 행사와 연결하여 행사 상세 화면에서도 관련 예산 정보를 확인할 수 있도록 설계했습니다.
- 통장 거래 내역 엑셀 파일을 업로드하면 기존 지출 내역과 자동으로 비교하여 매칭 여부를 판단하도록 구성했습니다.
- 매칭 결과를 목록에 반영하여 실제 통장 거래와 등록된 지출 내역의 정합성을 검증할 수 있도록 했습니다.
- QR 기반 모바일 등록 흐름을 제공하여 PC에서 지출 정보를 입력하다가 모바일로 영수증 사진 촬영을 이어서 진행할 수 있도록 했습니다.
- OCR을 활용하여 영수증 이미지에서 날짜, 금액, 가맹점 정보를 추출하고 지출 등록 과정의 입력 부담을 줄였습니다.

### 4.4 자산 관리

학생회가 보유한 물품의 현황과 대여 상태를 관리할 수 있습니다.  
카메라, 삼각대, 마이크, 스피커, 테이블 등 행사 운영에 필요한 자산을 등록하고, 대여 가능 여부와 대여 이력을 한눈에 확인할 수 있습니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/d0ea78fb-c8cb-4204-8fd3-f95909ac4a34" />

- 자산 목록 화면에서는 등록된 물품을 카드 형태로 확인할 수 있습니다.  
각 자산은 이미지, 자산명, 보관 위치, 재고 수량, 구매가, 태그, 대여 상태를 함께 보여주며, 검색과 상태 필터를 통해 필요한 물품을 빠르게 찾을 수 있습니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/df1de947-d780-45d7-be5a-400adfd91c2f" />

- 자산 등록 화면에서는 자산명, 카테고리, 보관 위치, 수량, 구매가, 태그, 사진, 설명을 입력할 수 있습니다.  
자산 사진을 함께 등록하여 실제 물품을 쉽게 식별할 수 있고, 태그를 통해 행사, 촬영, 음향, 야외 행사 등 사용 목적별로 분류할 수 있습니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/c4fdffce-5093-43b9-bf97-a7baf8a9b1b0" />

- 자산 상세 화면에서는 자산의 기본 정보와 현재 대여 가능 여부를 확인할 수 있습니다.  
카테고리, 보관 위치, 총 수량, 대여 가능 수량, 구매가, 설명, 태그를 함께 보여주며, 하단에는 해당 자산의 대여 이력을 표시합니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/6286b2c3-90e0-481c-a382-d8f1d7254d90" />

- 자산 수정 화면에서는 기존에 등록된 자산 정보를 변경할 수 있습니다.  
물품명, 카테고리, 보관 위치, 수량, 구매가, 태그, 사진, 설명을 수정할 수 있어 실제 자산 정보가 변경되었을 때 관리자가 최신 상태로 유지할 수 있습니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/6acf334f-148d-4599-91d4-7c743b263a9b" />

- 대여 처리 화면에서는 대여자명, 학번, 담당자, 수량, 대여일, 반납 예정일, 신분증 제출 여부, 비고를 입력하여 자산 대여를 기록할 수 있습니다.  
대여가 완료되면 해당 자산의 재고와 대여 가능 상태가 변경되고, 상세 화면의 대여 이력에도 반영됩니다.

구현 포인트

- 학생회 물품을 자산 단위로 등록하고, 보관 위치와 수량을 함께 관리할 수 있도록 구성했습니다.
- 자산 상태를 대여 가능, 대여 중, 사용 불가로 구분하여 현재 사용할 수 있는 물품을 빠르게 확인할 수 있도록 했습니다.
- 자산 이미지와 태그를 함께 저장하여 사용자가 물품을 쉽게 식별하고 목적별로 검색할 수 있도록 했습니다.
- 자산 상세 화면에서 총 수량과 대여 가능 수량을 분리하여 현재 재고 상태를 명확하게 확인할 수 있도록 했습니다.
- 대여 처리 시 대여자 정보, 담당자, 수량, 대여일, 반납 예정일, 신분증 제출 여부를 기록하여 대여 이력을 추적할 수 있도록 했습니다.
- 대여 및 반납 이력을 자산 상세 화면에 연결하여 물품 사용 기록을 누적 관리할 수 있도록 설계했습니다.
- 자산 수정 기능을 제공하여 보관 위치, 수량, 설명, 태그 등 변경되는 정보를 관리자가 지속적으로 갱신할 수 있도록 했습니다.

### 4.5 학생 관리

학생회비 납부 대상 학생 명단과 납부 현황을 관리할 수 있습니다.  
관리자는 전체 학생 수, 납부 완료 인원, 미납 인원, 납부율을 한눈에 확인하고, 학번·이름·학년·납부 여부 기준으로 학생 명단을 조회할 수 있습니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/ff51a22c-dd04-4635-85a0-f913369aae5a" />


- 학생 관리 화면에서는 전체 학생 명단과 학생회비 납부 상태를 함께 확인할 수 있습니다.  
상단 요약 카드에서는 전체 학생 수, 납부 완료 인원, 미납 인원, 납부율을 보여주며, 하단 목록에서는 학번, 이름, 학부/전공, 학년, 납부 여부, 납부일, 비고 정보를 확인할 수 있습니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/6dcdd9af-e1c3-45e8-bd79-7a72638e5f73" />

- 명단 업로드 기능을 통해 엑셀 파일 기반으로 학생 명단을 일괄 등록할 수 있습니다.  
업로드 과정은 파일 업로드, 컬럼 매핑, 미리보기, 완료 단계로 구성되어 있으며, 필수 컬럼인 학번, 이름, 학부/전공, 학년 정보를 기준으로 학생 데이터를 등록합니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/df892b91-f5b4-443c-b4b4-aba8fead8e50" />

- 납부 상태 필터를 통해 납부 완료 학생과 미납 학생을 구분하여 조회할 수 있습니다.  
미납 필터를 선택하면 아직 학생회비를 납부하지 않은 학생만 확인할 수 있어, 회비 납부 독려나 추가 확인이 필요한 대상을 빠르게 파악할 수 있습니다.

구현 포인트

- 학생 명단을 엑셀 파일로 일괄 업로드할 수 있도록 구성했습니다.
- 학생 데이터는 학번, 이름, 학부/전공, 학년 정보를 기준으로 관리합니다.
- 학생회비 납부 여부와 납부일을 함께 저장하여 납부 현황을 추적할 수 있도록 했습니다.
- 전체 학생 수, 납부 완료 인원, 미납 인원, 납부율을 집계하여 운영자가 현재 납부 상황을 빠르게 확인할 수 있도록 했습니다.
- 이름 또는 학번 검색 기능을 제공하여 특정 학생을 빠르게 찾을 수 있도록 했습니다.
- 학년 필터와 납부 상태 필터를 제공하여 필요한 학생 그룹만 조회할 수 있도록 구성했습니다.
- 명단 업로드 과정을 단계별로 분리하여 파일 업로드, 컬럼 매핑, 미리보기, 등록 완료 흐름을 안정적으로 처리할 수 있도록 설계했습니다.

### 4.6 설문조사

학생회 행사와 관련된 설문을 생성하고, 응답을 수집하며, 결과를 확인할 수 있습니다.  
관리자는 행사 만족도 조사, 메뉴 선호도 조사, 일정 조사 등 다양한 목적의 설문을 만들고 응답 링크를 공유할 수 있습니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/0598e899-54eb-4b59-9b45-440ecb586e4c" />

- 설문조사 목록 화면에서는 작성중, 진행중, 마감 상태의 설문을 확인할 수 있습니다.  
각 설문은 제목, 설명, 문항 수, 응답 수, 수정일, 작성자, 연결 행사 정보를 함께 보여주며, 설문 수정, 링크 복사, 마감 처리, 결과 확인 기능을 제공합니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/58660497-d5ac-486a-a1d3-f885272e9e55" />

- 설문 만들기 화면에서는 설문 제목, 설명, 연결 행사를 입력하고 문항을 추가할 수 있습니다.  
문항 유형은 단답형, 장문형, 객관식, 체크박스, 드롭다운으로 구성할 수 있어 설문 목적에 맞는 응답 형식을 선택할 수 있습니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/601e26a5-3017-4248-b75b-136ae83540a2" />

- 응답자는 공유된 설문 링크를 통해 설문에 참여할 수 있습니다.  
필수 문항은 별도로 표시되며, 객관식, 체크박스, 단답형 문항에 응답한 뒤 제출할 수 있습니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/2af52693-282f-4785-b7e4-b1a172fa325a" />

- 설문 결과 화면에서는 전체 응답 수, 마감 응답일, 문항 수를 확인할 수 있습니다.  
객관식이나 체크박스 문항은 응답 비율을 막대 형태로 보여주고, 단답형 문항은 응답 내용을 목록으로 확인할 수 있어 행사 운영 의사결정에 활용할 수 있습니다.

구현 포인트

- 설문을 작성중, 진행중, 마감 상태로 구분하여 설문 생명주기를 관리할 수 있도록 구성했습니다.
- 설문은 선택적으로 행사와 연결할 수 있어 행사 상세 화면에서도 관련 설문을 함께 확인할 수 있도록 설계했습니다.
- 단답형, 장문형, 객관식, 체크박스, 드롭다운 등 다양한 문항 유형을 지원하도록 구성했습니다.
- 설문 문항과 응답 데이터를 분리하여 설문 구조와 사용자 응답을 독립적으로 관리할 수 있도록 했습니다.
- 설문 링크 복사 기능을 통해 외부 사용자도 설문에 쉽게 접근할 수 있도록 했습니다.
- 설문 결과 화면에서는 문항별 응답 수와 응답 비율을 집계하여 관리자가 결과를 빠르게 해석할 수 있도록 했습니다.
- 마감된 설문은 결과 확인 중심으로 제공하여 응답 수집 단계와 결과 분석 단계를 명확히 분리했습니다.

### 4.7 워크스페이스

학생회 부서별 자료와 회의록을 관리할 수 있는 협업 공간입니다.  
전체 워크스페이스와 부서별 워크스페이스를 구분하여 운영할 수 있으며, 각 부서에서 필요한 회의록과 공유 자료를 한 곳에서 관리할 수 있습니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/433d4489-9de6-438d-8567-9d17fca6eda8" />

- 워크스페이스 목록 화면에서는 전체 워크스페이스와 회장단, 총무부, 복지국, 기획국, 홍보국 등 부서별 워크스페이스를 확인할 수 있습니다.  
각 워크스페이스 카드에는 부서명, 설명, 파일 수, 회의록 수, 최근 업데이트 일자가 표시되어 부서별 자료 관리 현황을 빠르게 파악할 수 있습니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/0b315cb7-6522-4407-8230-9830c52d8482" />

- 워크스페이스 상세 화면에서는 회의록과 공유 자료를 탭으로 구분하여 확인할 수 있습니다.  
회의록 목록에서는 회의 제목, 작성일, 참석자, 연결 행사, 주요 내용을 함께 보여주며, 회의록 작성 버튼을 통해 새로운 회의 내용을 등록할 수 있습니다.

<img width="1512" height="982" alt="image" src="https://github.com/user-attachments/assets/04b14bd4-31b4-4884-81ca-cd899ac05d35" />

- 회의록 작성 화면에서는 회의 제목, 회의 날짜, 참석자, 안건, 회의 내용, 연결 행사를 입력할 수 있습니다.  
회의록을 특정 행사와 연결하면 행사 상세 화면에서도 관련 회의록을 함께 확인할 수 있어, 행사 준비 과정에서 논의된 내용을 추적할 수 있습니다.

구현 포인트

- 전체 워크스페이스와 부서별 워크스페이스를 분리하여 공용 자료와 부서별 자료를 구분해 관리할 수 있도록 구성했습니다.
- 각 워크스페이스별로 파일 수, 회의록 수, 최근 업데이트 일자를 집계하여 자료 관리 현황을 한눈에 확인할 수 있도록 했습니다.
- 회의록과 공유 자료를 탭으로 분리하여 워크스페이스 내 자료 유형을 명확하게 구분했습니다.
- 회의록 작성 시 제목, 날짜, 참석자, 안건, 회의 내용을 함께 기록하여 회의 진행 내용을 구조적으로 저장할 수 있도록 했습니다.
- 회의록을 특정 행사와 연결할 수 있도록 하여 행사 상세 화면에서도 관련 회의 내용을 함께 확인할 수 있는 구조로 설계했습니다.
- 부서별 워크스페이스를 통해 학생회 내부의 부서 단위 협업과 자료 관리를 지원하도록 구성했습니다.

### 4.8 일정 관리

학생회 운영에 필요한 행사 일정과 시간 조율 일정을 한 화면에서 관리할 수 있습니다.  
사용자는 캘린더를 통해 월별 일정을 확인할 수 있고, 특정 일정에 대한 가능한 시간을 구성원들과 조율할 수 있습니다.

<img width="1536" height="976" alt="image" src="https://github.com/user-attachments/assets/46718e01-d95d-488f-b1dd-241046ec3aaa" />

- 캘린더 화면에서는 행사와 워크스페이스 회의 일정을 구분하여 표시합니다.  
날짜를 클릭하면 해당 날짜의 상세 일정을 확인할 수 있으며, 월별 일정 목록을 통해 전체 행사 흐름을 빠르게 파악할 수 있습니다.

<img width="1536" height="806" alt="image" src="https://github.com/user-attachments/assets/2d1a289b-79d6-4171-b24d-b7922a45c4d7" />

- 시간 조율 탭에서는 진행 중인 일정 조율 목록을 확인할 수 있습니다.  
각 일정은 진행 상태, 조율 기간, 가능 시간 범위, 응답 인원 수를 함께 보여주며, 사용자는 일정별로 응답하거나 결과를 확인할 수 있습니다.

<img width="1536" height="1361" alt="image" src="https://github.com/user-attachments/assets/9cceeabe-3574-43d2-8346-795424cb262f" />

- 일정 조율 상세 화면에서는 참여자가 가능한 시간을 직접 선택할 수 있습니다.  
날짜와 시간대가 격자 형태로 제공되며, 사용자는 셀을 클릭하거나 드래그하여 가능한 시간을 선택한 뒤 응답을 저장할 수 있습니다.

구현 포인트

- 캘린더 기반 일정 조회와 시간 조율 기능을 분리하여 사용 목적에 맞게 화면을 구성했습니다.
- 행사 일정과 워크스페이스 회의 일정을 구분하여 표시할 수 있도록 일정 유형을 분리했습니다.
- 일정 조율 기능에서는 날짜 범위, 시간 범위, 시간 단위 정보를 기반으로 선택 가능한 시간표를 동적으로 구성했습니다.
- 참여자는 가능한 시간대를 선택하여 응답할 수 있고, 관리자는 응답 인원 수와 조율 결과를 확인할 수 있습니다.
- 일정 상태를 진행중, 마감 등으로 구분하여 현재 응답 가능한 일정과 종료된 일정을 명확히 구분했습니다.

### 4.9 수정 이력

서비스 내 주요 데이터의 생성, 수정, 삭제, 업로드, 매칭 기록을 확인할 수 있습니다.  
관리자는 어떤 사용자가 언제 어떤 데이터를 변경했는지 추적할 수 있으며, 행사, 조직 멤버, 예산, 파일 등 주요 도메인의 변경 내역을 감사 로그 형태로 확인할 수 있습니다.

<img width="1536" height="998" alt="image" src="https://github.com/user-attachments/assets/fe7d4c0f-09f1-4785-a72b-d74b5eda6322" />

수정 이력 화면에서는 주요 데이터 변경 내역을 시간순으로 확인할 수 있습니다.  
각 이력에는 작업자, 작업 시간, 기능 영역, 대상 데이터, 작업 유형이 표시되며, 변경 전 값과 변경 후 값을 함께 확인할 수 있습니다.

필터 기능을 통해 특정 기능, 작업 유형, 사용자 ID, 기간 조건으로 이력을 조회할 수 있습니다.  
이를 통해 행사 상태 변경, 조직 멤버 삭제, 예산 등록, 파일 업로드, 통장 매칭 등 특정 작업 기록만 선별해서 확인할 수 있습니다.

구현 포인트

- 주요 도메인 데이터의 생성, 수정, 삭제, 업로드, 매칭 작업을 이력으로 남길 수 있도록 구성했습니다.
- 작업자, 작업 시간, 기능 영역, 대상 데이터, 작업 유형을 함께 기록하여 변경 내역의 추적 가능성을 높였습니다.
- 변경 전 값과 변경 후 값을 함께 저장하여 데이터가 어떤 값에서 어떤 값으로 변경되었는지 확인할 수 있도록 했습니다.
- 기능별, 작업 유형별, 사용자별, 기간별 필터를 제공하여 필요한 이력을 빠르게 조회할 수 있도록 구성했습니다.
- 행사 상태 변경, 조직 멤버 삭제, 통장 매칭 결과 등 운영상 중요한 작업을 감사 로그로 남겨 관리자가 사후 확인할 수 있도록 했습니다.
- 데이터 삭제나 상태 변경처럼 되돌리기 어려운 작업도 이력으로 남겨 운영 중 발생할 수 있는 문제를 추적할 수 있도록 설계했습니다.

### 4.10 조직 관리

학생회 조직의 멤버, 부서, 권한 정보를 관리할 수 있습니다.  
관리자는 현재 조직에 속한 구성원을 확인하고, 각 멤버의 소속 부서와 권한을 설정하여 학생회 내부 역할에 맞게 접근 범위를 관리할 수 있습니다.

<img width="1536" height="998" alt="image" src="https://github.com/user-attachments/assets/ce987de5-30fe-4a78-94ba-7566d65bcda7" />

조직 관리 화면에서는 전체 멤버 수, 회장 수, 국장 수, 국원 수를 요약해서 확인할 수 있습니다.  
하단 목록에서는 구성원의 이름, 학번, 부서, 권한, 가입일을 확인할 수 있으며, 이름이나 학번으로 특정 멤버를 검색할 수 있습니다.

부서 필터와 권한 필터를 통해 특정 부서 또는 특정 권한을 가진 멤버만 조회할 수 있습니다.  
관리자는 멤버별 수정 버튼을 통해 구성원의 부서와 권한을 변경할 수 있고, 초대 코드를 통해 새로운 멤버를 조직에 초대할 수 있습니다.

구현 포인트

- 조직 멤버를 이름, 학번, 부서, 권한, 가입일 기준으로 관리할 수 있도록 구성했습니다.
- 전체 멤버 수와 권한별 인원 수를 집계하여 조직 구성을 한눈에 확인할 수 있도록 했습니다.
- 부서와 권한을 분리하여 학생회 내부 역할에 따라 접근 범위를 다르게 관리할 수 있도록 설계했습니다.
- 이름과 학번 검색 기능을 제공하여 특정 멤버를 빠르게 찾을 수 있도록 했습니다.
- 부서별, 권한별 필터를 제공하여 필요한 조직 구성원만 선별해서 조회할 수 있도록 구성했습니다.
- 멤버별 수정 기능을 통해 조직 내 역할 변경이나 부서 이동을 관리할 수 있도록 했습니다.
- 초대 코드 기반으로 새로운 멤버가 조직에 합류할 수 있는 흐름을 제공했습니다.
- 조직 탈퇴 기능을 제공하여 사용자가 더 이상 해당 조직에 속하지 않을 경우 멤버십을 종료할 수 있도록 했습니다.
