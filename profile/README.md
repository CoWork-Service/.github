...
| SSO | GET | `/api/auth/sso/callback` | SSO 콜백 |
| SSO | GET | `/api/auth/sso/profile` | SSO 임시 프로필 조회 |
| SSO | POST | `/api/auth/sso/register` | SSO 회원가입 |
| 약관 | GET | `/api/auth/consent` | 약관 동의 상태 조회 |
| 약관 | POST | `/api/auth/consent` | 약관 동의 저장 |
| 기수 | GET | `/api/cohorts` | 기수 목록 조회 |
| 기수 | POST | `/api/cohorts` | 기수 생성 |
| 기수 | PUT | `/api/cohorts/{id}` | 기수 수정 |
| 조직 멤버 | GET | `/api/org/members` | 조직 멤버 목록 |
| 조직 멤버 | PUT | `/api/org/members/{id}` | 멤버 역할/부서 수정 |
| 조직 멤버 | POST | `/api/org/pending/{userId}/approve` | 가입 승인 |
| 조직 멤버 | POST | `/api/org/pending/{userId}/reject` | 가입 거절 |
| 조직 부서 | GET | `/api/org/departments` | 부서 목록 조회 |
| 조직 부서 | PUT | `/api/org/departments` | 부서 목록 수정 |
| 행사 | GET | `/api/events` | 행사 목록 조회 |
| 행사 | POST | `/api/events` | 행사 생성 |
| 행사 | GET | `/api/events/{id}` | 행사 상세 조회 |
| 행사 | PUT | `/api/events/{id}` | 행사 수정 |
| 행사 | DELETE | `/api/events/{id}` | 행사 삭제 |
| 행사 사진 | POST | `/api/events/{id}/photos` | 행사 사진 업로드 |
| 행사 사진 | DELETE | `/api/events/{id}/photos/{photoId}` | 행사 사진 삭제 |
| 예산 | GET | `/api/expenses` | 지출 목록 조회 |
| 예산 | GET | `/api/expenses/summary` | 지출 요약 |
| 예산 | POST | `/api/expenses` | 지출 등록 |
| 예산 | GET | `/api/expenses/{id}` | 지출 상세 조회 |
| 예산 | PUT | `/api/expenses/{id}` | 지출 수정 |
| 예산 | DELETE | `/api/expenses/{id}` | 지출 삭제 |
| 예산 OCR | POST | `/api/expenses/ocr` | 영수증 OCR |
| 예산 OCR | POST | `/api/expenses/match-receipt` | 영수증/통장 내역 매칭 |
| 예산 OCR | POST | `/api/expenses/parse-excel` | 통장/엑셀 내역 파싱 |
| 학생 | GET | `/api/students` | 학생 목록 조회 |
| 학생 | GET | `/api/students/summary` | 회비 요약 |
| 학생 | POST | `/api/students` | 학생 등록 |
| 학생 | POST | `/api/students/import` | 학생 엑셀 업로드 |
| 학생 | PUT | `/api/students/{id}` | 학생 수정 |
| 학생 | DELETE | `/api/students/{id}` | 학생 삭제 |
| 학생 | PATCH | `/api/students/bulk-payment` | 회비 일괄 처리 |
| 메모 | GET | `/api/memos` | 메모 목록 조회 |
| 메모 | POST | `/api/memos` | 메모 생성 |
| 메모 | PUT | `/api/memos/{id}` | 메모 수정 |
| 메모 | PATCH | `/api/memos/{id}/status` | 메모 상태 변경 |
| 메모 | PATCH | `/api/memos/{id}/priority` | 메모 중요도 변경 |
| 파일 | GET | `/api/files` | 파일/폴더 목록 |
| 파일 | POST | `/api/files/folder` | 폴더 생성 |
| 파일 | POST | `/api/files/upload` | 파일 업로드 |
| 파일 | GET | `/api/files/{id}/download` | 파일 다운로드 |
| 파일 | PATCH | `/api/files/{id}/move` | 파일/폴더 이동 |
| 워크스페이스 | GET | `/api/workspaces` | 워크스페이스 목록 |
| 회의록 | GET | `/api/workspaces/{id}/meetings` | 회의록 목록 |
| 회의록 | POST | `/api/workspaces/{id}/meetings` | 회의록 생성 |
| 회의록 | PUT | `/api/workspaces/{id}/meetings/{meetingId}` | 회의록 수정 |
| 설문 | GET | `/api/surveys` | 설문 목록 조회 |
| 설문 | POST | `/api/surveys` | 설문 생성 |
| 설문 | GET | `/api/surveys/{id}` | 설문 상세 조회 |
| 설문 | PATCH | `/api/surveys/{id}/status` | 설문 상태 변경 |
| 설문 | POST | `/api/surveys/{id}/respond` | 설문 응답 제출 |
| 설문 | GET | `/api/surveys/{id}/results` | 설문 결과 조회 |
| 일정 조율 | GET | `/api/timetables` | 조율표 목록 |
| 일정 조율 | POST | `/api/timetables` | 조율표 생성 |
| 일정 조율 | POST | `/api/timetables/{id}/respond` | 가능 시간 제출 |
| 일정 조율 | GET | `/api/timetables/{id}/results` | 가능 시간 결과 |
| 자산 | GET | `/api/assets` | 자산 목록 조회 |
| 자산 | POST | `/api/assets` | 자산 등록 |
| 자산 | POST | `/api/assets/{id}/rent` | 자산 대여 |
| 자산 | PATCH | `/api/assets/{id}/rentals/{rentalId}/return` | 자산 반납 |
| 모바일 | POST | `/api/mobile/sessions` | 모바일 업로드 세션 생성 |
| 모바일 | POST | `/api/mobile/sessions/{token}/upload` | 모바일 사진 업로드 |
| 모바일 | POST | `/api/mobile/sessions/{token}/ocr` | 모바일 OCR |
| 모바일 | POST | `/api/mobile/sessions/{token}/expense` | 모바일 지출 생성 |
| 감사 로그 | GET | `/api/audit-logs` | 감사 로그 조회 |
| 감사 로그 | GET | `/api/audit-logs/recent` | 최근 감사 로그 조회 |
